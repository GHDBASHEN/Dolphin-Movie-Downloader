...
const DOWNLOADS_DB = path.join(
  app.getPath("userData"),
  "active_downloads.json"
);
...
function saveState() {
    // FILTER: Only save torrents that are NOT complete (progress < 1)
    const active = client.torrents
        .filter(t => t.progress < 1) // <--- THIS REMOVES COMPLETED ITEMS FROM SAVE FILE
        .map(t => ({
            id: t.uiId,
            title: t.uiTitle,
            magnet: t.magnetURI,
    ...
    try {
        ...
        console.log("✅ Saved state:", active.length, "downloads (completed ones cleared).");
...
function createWindow() {
  ...
  const sendStatus = (text) => {
    if (splash && !splash.isDestroyed()) {
      try {
        splash.webContents.send("update-status", text);
      } catch (e) {}
    }
  };
  ...
  autoUpdater.on("download-progress", (p) =>
    sendStatus(`Downloading: ${p.percent.toFixed(0)}%`)
  );
...
ipcMain.handle("search-movies", async (event, query) => {
  try {
    ...
    return results.filter((t) => {
      const isVideo =
        /1080p|720p|480p|BluRay|WEBRip|H.264|x265|AVI|MKV|MP4/i.test(t.title);
...
async function startTorrent(event, torrentData, isRestore = false) {
  ...
  const downloadLocation =
    isRestore && torrentData.path ? torrentData.path : config.downloadPath;
  ...
  client.add(magnet, { path: downloadLocation }, (torrent) => {
    ...
    torrent.on("done", () => {
      // 1. Mark as completed in the internal list?
      // (Optional, but good for logic. For now, we just notify UI)
      ...
      let fullPath = "";
      if (file) {
        fullPath = path.join(downloadLocation, file.path);
      }

      // ONLY Send the message. DO NOT call client.remove() here!
...
ipcMain.on("preview-file", (event, id) => {
  const torrent = client.torrents.find((t) => t.uiId === id);
  if (!torrent) return;

  // Check if files are ready (metadata fetched)
  if (!torrent.files || torrent.files.length === 0) return;

  // Find the largest video file (avoids sample files)
  const file = torrent.files
    .sort((a, b) => b.length - a.length) // Sort by size descending
    .find((f) => f.name.match(/\.(mp4|mkv|avi)$/i));

  if (file) {
    // Construct the full path to the file on disk
    const fullPath = path.join(torrent.path, file.path);

    console.log("🎬 Previewing:", fullPath);
